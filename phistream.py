...
# ============================================================
class Rule:
    def __init__(self, field=None, operator=None, value=None, expr=None, severity="error", name=None):
        self.field = field
    ...
    def validate(self, columns):
        for r in self.rules:
            if r.field and r.field not in columns:
                raise ValueError(f"Column not found: {r.field}")
    ...
    def evaluate(self, df):
        ...
        for r in self.rules:
            ...
            if r.severity == "error":
                ...
                warn |= m
            # Convert boolean mask to string reason safely
            reason = reason.where(~m, reason + f"{r.name};")
...
class HLL:
    ...
    def add_batch(self, series):
        # Optimized vectorized hashing for the entire chunk
        hashes = series.map(hash)
        for h in hashes:
            b = h & (self.buckets - 1)
            w = h >> 8
...
# ============================================================
# Stats Detector (Local Chunk Anomalies)
# ============================================================
class StatsDetector:
    ...
    def detect(self, df):
        ...
        for c in df.columns:
            ...
            if mad > 0:
                z_scores = 0.6745 * (df[c] - med) / mad
            else:
                std = df[c].std()
                if std == 0 or np.isnan(std):
                    continue
                z_scores = (df[c] - df[c].mean()) / std
            
            m = np.abs(z_scores) > self.z
            mask |= m
            reason = reason.where(~m, reason + f"stat_{c};")
...
class Profiler:
    ...
    def update(self, df):
        for c in df.columns:
            ...
            # Use the optimized batch add for HyperLogLog
            st["unique"].add_batch(s.dropna())
            
            if pd.api.types.is_numeric_dtype(s):
                st["numeric"].update(s)
                
            # Update top 5 incrementally (approximation for streaming)
            current_top = s.value_counts().head(5).to_dict()
            st["top"] = current_top # Note: strict streaming top-K requires count-min sketch, this is a local approximation
    ...
    def export(self):
        ...
        for c, st in self.stats.items():
            ...
            rs = st["numeric"]
            if rs.n > 0:
                o.update({"min": rs.min, "max": rs.max,
...
class Engine:
    ...
    def auto_schema(self, path):
        ...
        return Schema({"columns": list(s.columns),
    ...
    def run(self, path):
        ...
        reader = pd.read_csv(open_file(path),
        ...
        for chunk in reader:
            ...
            if len(self.samples) < 100 and not bad_rows.empty:
                self.samples.extend(bad_rows.head(100 - len(self.samples)).to_dict("records"))
                
            writer.write(good, bad_rows)
            self.profiler.update(chunk.drop(columns=["__error_reason"], errors="ignore"))
            
            total += len(chunk)
            clean += len(good)
            bad += len(bad_rows)
            
            logging.info(f"chunk_rows={len(chunk)} time={time.time()-c0:.2f}s")
        ...
        report = {
            ...
            "rows_clean": clean,
            "rows_anomalies": bad,
            "rows_per_sec": int(total / (time.time() - t0)) if time.time() - t0 > 0 else 0,
